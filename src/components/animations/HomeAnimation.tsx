...
const HomeAnimation = () => {
    ...
    const sketch = (p: p5) => {
      ...
      p.draw = () => {
        ...
        for (let i = particles.length - 1; i >= 0; i--) {
          ...
          particle.x += particle.vx + p.sin(time + particle.y * 0.01) * 0.5;
          particle.y += particle.vy;
          particle.life++;
          
          // Fade in and out
          const alpha = particle.life < particle.maxLife * 0.1 
            ? particle.life / (particle.maxLife * 0.1)
            : particle.life > particle.maxLife * 0.9
            ? 1 - (particle.life - particle.maxLife * 0.9) / (particle.maxLife * 0.1)
            : 1;
          
          // Draw as organic shapes representing nature
          p.fill(25, 95, 53, alpha * 0.15); // Orange from CSS
          p.noStroke();
  ...
  return <div ref={sketchRef} className="absolute inset-0 opacity-20" />;
};
